...
# This line is often necessary for environments that have SSL/certificate issues
...
# --- NEW: SendGrid Configuration and Email Function (FROM test.py & config.ini) ---
def get_sendgrid_config():
    """Reads SendGrid configuration from config.ini."""
    config = configparser.ConfigParser()
    try:
        # ATTEMPT TO READ FILE AND STORE THE LIST OF FILES SUCCESSFULLY READ
        files_read = config.read("config.ini") 
        
        # --- DEBUG CHECK FOR config.ini ---
        if not files_read:
            print("\n--- DEBUG MESSAGE ---")
            # If the list is empty, the file was not found
            print("CRITICAL: 'config.ini' NOT FOUND or failed to load content.")
            print("Current CWD:", os.getcwd())
            print("---------------------\n")
            return None, None
        
        # If the file was read, proceed to access the settings section
        settings = config["settings"]
        # Reads APIKEY and FROM email
        return settings.get("APIKEY", None), settings.get("FROM", None) 
        
    except Exception as e:
        # This will catch the 'settings' key error if the file was read but was empty/malformed
...
def send_email(to_email, subject, body):
    ...
    The 'to_email' argument is the specific user's email for the reminder.
    """
    API, from_email = get_sendgrid_config()
    user_name = st.session_state.user.get('name', 'StudyCircle User')

    if not API or not from_email:
        print("❌ SendGrid API key or FROM email not configured in config.ini.")
        st.error("Email service is temporarily unavailable (Configuration missing).")
        return False # Return False on failure

    # Ensure to_email is in the correct list format for the Mail constructor
    if isinstance(to_email, str):
        to_emails = [to_email]
    elif isinstance(to_email, list):
        to_emails = to_email
    else:
        print("Invalid to_email format.")
        return False
    ...
    <body>
        <p>Hello {user_name},</p>
        <p>This is a friendly reminder from **StudyCircle**.</p>
        <p><strong>Time:</strong> {datetime.now().strftime('%b %d, %I:%M %p')}</p>
        <p><strong>Reminder:</strong> {body}</p>
        <p>Keep up the great work!</p>
        <p>Best regards,<br>The StudyCircle Team</p>
    </body>
    </html>
    """

    # Use the prepared list of emails (which contains only the current user's email)
    message = Mail(
    ...
    try:
        ...
        # Check if the SendGrid call was successful (2xx status code)
        if 200 <= response.status_code < 300:
            # We don't print the whole list, just acknowledge success
            print(f"✅ Email sent to {to_email} via SendGrid. Status: {response.status_code}")
            return True
        else:
            print(f"❌ SendGrid Error. Status: {response.status_code}, Body: {response.body.decode('utf-8') if response.body else 'No body'}")
            return False

    except Exception as e:
        print(f"❌ SendGrid General Error: {e}")
        return False
        
# --- END NEW: SendGrid Configuration and Email Function ---
...
def apply_custom_css():
    st.markdown("""
        <style>
            ...
            .stApp {
                ...
                background: url("https://media.istockphoto.com/id/2168676828/vector/print.jpg?s=612x612&w=0&k=20&c=OojvH8neotmc_8-IicCKgBLbbGi0lFDuMyGz6pRcD0c=") no-repeat center center fixed;
                background-size: cover;
            }
 ...
 /*
 * MODIFIED CSS FOR A PERFECTLY BALANCED 3 + 3 LAYOUT
 */
.feature-grid {
    display: grid;
    /* Define 3 equal columns for all six cards */
    grid-template-columns: repeat(3, 1fr); 
    gap: 30px; /* Consistent spacing between cards */
    
    /* Center the entire set of cards horizontally */
    justify-content: center; 
    
    /* Ensures all cards stretch to the same height for uniform appearance */
    align-items: stretch; 
}

/* Ensure individual cards reset any previous explicit column positioning */
.feature-grid .feature-card {
    grid-column: auto !important; 
    /* The 3x3 grid will automatically handle the flow */
    text-align: center;
}


/* * CRITICAL: Media Query for Responsiveness 📱
 * Ensures a single-column layout on small screens (like mobile phones)
 */
...
def get_current_reminders(user_id, subject):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, message, scheduled_time, email_sent 
            FROM reminders 
...
def update_email_sent_status(reminder_id):
...
# --- NEW Alert System Function with Email Logic (Fix F841 applied here) ---
def check_for_due_reminders(user_id, subject):
    """
    Checks for reminders due within the next hour,
    displays an on-screen alert, and sends an email to the user who set the reminder.
    """
    # Fetch all *upcoming* reminders (scheduled_time > now)
    reminders = get_current_reminders(user_id, subject) 
    now = datetime.now()
    # Get the email of the logged-in user who set the reminder
    user_email = st.session_state.user.get('email')
    user_name = st.session_state.user.get('name', 'User')

    for reminder in reminders:
        # Check if reminder is due within the next 1 hour AND email hasn't been sent
        if now < reminder['scheduled_time'] < now + timedelta(hours=1) and not reminder['email_sent']:
            # Display on-screen alert - FIX APPLIED: user_name is used here to fix Ruff F841
            st.markdown(f"""
                <div class="glass-container alert-glow">
                    <div class="stWarning">
                        <i class="fa-solid fa-bell"></i> <strong>Heads up, {user_name}! You have a reminder coming up soon:</strong>
                        <br>
            ...
            email_body = reminder['message'] 
            
            # Use the new SendGrid send_email function, passing the specific user email
            success = send_email(to_email=user_email, subject=email_subject, body=email_body)
            
            if success:
                # Use the reminder's ID for updating
                update_email_sent_status(reminder['id'])
        ...
        <div class="glass-container fade-in">
            <div class="feature-grid">
                ...
                <div class="feature-card">
                 ...
                 <div class="feature-card">
            <div class="icon xl class-fa fa-solid fa-user-plus"></div>
            <h4>Peer Matcher</h4>
            <p>Connect instantly with students having similar courses or goals.</p>
        </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
